/* ============================================================
   luhn. — генератор номеров карт
   Чистый JS, без зависимостей. Работает локально из файла.
   ============================================================ */
...
const BRANDS = {
  visa: {
    ...
    mark: "Visa",
    length: 16,
    // случайный префикс: первая цифра 4
    randomPrefix: () => "4",
  ...
  mastercard: {
    ...
    randomPrefix: () => {
      if (Math.random() < 0.5) {
        return String(50 + Math.floor(Math.random() * 5) + 1); // 51–55
      }
      return String(2221 + Math.floor(Math.random() * 500)); // 2221–2720
    },
  ...
  mir: {
    ...
    mark: "Mir",
    length: 16,
    // реальный диапазон МИР — 2200–2204
...
/**
 * Контрольная цифра для тела номера.
 * В финальном номере контрольная цифра стоит справа и не удваивается,
 * поэтому правая цифра тела в сумме удваивается (чётность сдвинута).
 */
...
/**
 * Генерирует валидный по Луна номер для выбранной системы.
 * Если задан BIN — номер начинается с него.
...
function generateNumber(brand, bin) {
  ...
  for (let attempt = 0; attempt < 100; attempt++) {
    // добиваем до length-1 случайными цифрами, последняя — контрольная
...
/* ---------- определение системы по префиксу ---------- */
...
function randomExpiry() {
  ...
  const plusYears = 2 + Math.floor(Math.random() * 4); // 2–5 лет вперёд
...
/* ---------- сегментированный выбор системы ---------- */
...
/* ---------- плавающий индикатор активного сегмента ---------- */
...
/* ---------- лёгкий tilt карты за курсором ---------- */
/* отключено на touch-устройствах и при prefers-reduced-motion */
...
function validateBin() {
  ...
  if (detected && detected !== state.brand) {
    note.textContent = `This prefix belongs to ${BRANDS[detected].label} — numbers will still be generated with it`;
  } else if (!detected) {
    note.textContent = "This prefix doesn't match a known network, but will work for generation";
  } else {
    note.hidden = true;
    return raw;
  }
...
function commitQty() {
  ...
  if (n === null || n < 1 || n > MAX_QTY) {
    showQtyNote(`Enter a number from 1 to ${MAX_QTY}`);
    return false;
  }
  state.qty = n;
  renderQty();
...
// прямое поле ввода: только цифры, живая валидация
els.qtyInput.addEventListener("input", () => {
  ...
  if (n === null) return; // пустое поле — ждём ввода
  if (n < 1 || n > MAX_QTY) {
    showQtyNote(`Enter a number from 1 to ${MAX_QTY}`);
    return;
  }
...
/**
 * Метка бренда — стилизованный текст (без SVG и сети).
 */
function buildBrandMark(brandKey) {
  ...
  text.textContent = BRANDS[brandKey].mark || BRANDS[brandKey].label;
...
/* инлайн-состояние «Copied» на кнопке Copy all */
...
/* инлайн-состояние на подписи под картой */
...
/* токен отмены рендера: новая генерация останавливает старую */
let renderToken = 0;
...
function renderResults(cards) {
  ...
  els.resultsCount.textContent = cards.length ? `${cards.length} numbers` : "";

  const large = cards.length > 150;
  list.classList.toggle("is-large", large);

  // больше 10 строк — список получает собственную прокрутку
  ...
  const buildRow = (card, index) => {
    ...
    copyBtn.title = "Copy number, expiry and CVV";
  ...
  // чанкованная отрисовка: пачки по CHUNK строк за кадр, UI остаётся отзывчивым
  const step = () => {
    if (token !== renderToken) return; // отменено новой генерацией
    ...
    if (i < cards.length) {
      if (cards.length > 200) {
        els.resultsCount.textContent = `Rendering… ${i}/${cards.length}`;
      }
...
/* текстовые значения строки (для копирования) */
...
async function copyText(text) {
  try {
    ...
    // запасной путь для file:// и старых браузеров
    try {
...
els.exportBtn.addEventListener("click", () => {
  if (!state.lastCards.length) return;
  // полные данные на строку: номер|срок|CVV — файл самодостаточен для тестов
...
/* ---------- копирование номера по клику на карту ---------- */
...
function generate(opts) {
  ...
  if (pulse) {
    // деликатный пульс-блик на кнопке
  ...
  if (bin === null && els.binInput.value.trim() !== "") {
    return; // BIN некорректен — не генерируем
  }
...
// Enter в поле BIN тоже запускает генерацию
...
document.addEventListener("keydown", (e) => {
  // Ctrl/Cmd+Enter — генерация из любого места
  ...
  // Esc в поле BIN — очистить и снять фокус
...
/* ---------- инициализация ---------- */
  ...
  // мгновенный пример — страница не пустая, пользователь сразу видит результат
